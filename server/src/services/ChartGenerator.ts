import { ChartDataPoint, StockQuote } from '../types.js';

export function generateCalibratedChartData(
  symbol: string,
  range: string,
  quote?: Partial<StockQuote> | null
): ChartDataPoint[] {
  const currentPrice = Number(quote?.price || 150);
  const prevClose = Number(quote?.previousClose || currentPrice * 0.98);
  const dayOpen = Number(quote?.open || prevClose);
  const dayHigh = Number(quote?.high || Math.max(currentPrice, prevClose) * 1.015);
  const dayLow = Number(quote?.low || Math.min(currentPrice, prevClose) * 0.985);
  const totalVol = Number(quote?.volume || 1500000);

  const cleanRange = (range || '1D').toUpperCase();
  const points: ChartDataPoint[] = [];

  let numPoints = 78; // Default for 1D (5-minute bars in 6.5 hour trading day)
  ...
  switch (cleanRange) {
    ...
    case '5D':
      numPoints = 65; // ~13 points per day
      timeStepMs = 30 * 60 * 1000;
      startPrice = prevClose * (1 - (quote?.changePercent ? (quote.changePercent / 100) * 0.5 : 0.02));
      endPrice = currentPrice;
    ...
    case '1M':
    case '1MO':
      numPoints = 30; // 30 daily bars
      timeStepMs = 24 * 60 * 60 * 1000;
      startPrice = currentPrice * (1 - (quote?.changePercent ? (quote.changePercent / 100) * 2 : 0.05));
      endPrice = currentPrice;
    ...
    case '12M':
      numPoints = 250;
      timeStepMs = 24 * 60 * 60 * 1000;
      startPrice = Number(quote?.fiftyTwoWeekLow || currentPrice * 0.80);
      maxPrice = Number(quote?.fiftyTwoWeekHigh || currentPrice * 1.30);
      minPrice = Number(quote?.fiftyTwoWeekLow || currentPrice * 0.75);
      endPrice = currentPrice;
      break;
  ...
  // Generate pseudo-deterministic seed based on symbol name
  let seed = 0;
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    // Harmonic wave for realistic market oscillations
    ...
    // U-shaped intraday volume or steady daily volume
    let volWeight = 1.0;
    if (cleanRange === '1D') {
      const uShape = Math.pow(progress - 0.5, 2) * 4; // High at ends, lower in middle
