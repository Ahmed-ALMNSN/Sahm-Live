...
export const LiveStockTable: React.FC<LiveStockTableProps> = ({
  ...
  const filteredAndSortedStocks = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortField === 'mfi') {
        valA = a.mfi ?? calculateStockMfi(a.price, a.dayHigh, a.dayLow, a.open, a.previousClose, a.volume, a.changePercent).mfi;
        valB = b.mfi ?? calculateStockMfi(b.price, b.dayHigh, b.dayLow, b.open, b.previousClose, b.volume, b.changePercent).mfi;
      }
    ...
    <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden flex flex-col font-sans transition-colors duration-200">
      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row gap-3.5 justify-between items-stretch lg:items-center bg-white dark:bg-[#161b22] transition-colors duration-200">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 lg:pb-0 scrollbar-none">
          ...
          {onClearAllAlerts && stocks.length > 0 && (
            <button
              id="btn-clear-all-alerts"
              onClick={() => setIsConfirmingClearAlerts(true)}
              className="px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-400 border border-amber-500/30 transition-all flex items-center gap-1.5 font-sans"
              title={t.actions.clearAllAlerts}
            >
              <BellOff className="w-3.5 h-3.5" />
              <span>{t.actions.clearAllAlerts}</span>
              {stocks.filter(s => s.upperAlert !== null || s.lowerAlert !== null).length > 0 && (
                <span className="text-[10px] px-1.5 py-0.5 rounded font-mono font-bold bg-amber-500/20 text-amber-800 dark:text-amber-300">
                  {stocks.filter(s => s.upperAlert !== null || s.lowerAlert !== null).length}
                </span>
              )}
            </button>
          )}
        ...
        <table className="w-full text-left rtl:text-right border-collapse min-w-[960px]">
          ...
          <thead className="bg-slate-50 dark:bg-[#1a202c] text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider sticky top-0 z-10 border-b border-slate-200 dark:border-slate-700 font-sans transition-colors duration-200">
            <tr>
              ...
              {/* Money Flow Index (MFI) Column Header */}
              <th 
                className="py-3.5 px-3 cursor-pointer hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors whitespace-nowrap min-w-[130px]"
                onClick={() => handleSort('mfi')}
                title={lang === 'ar' ? 'مؤشر تدفق السيولة (Money Flow Index) - نسبة ضغط الشراء وحركة السيولة اللحظية' : 'Money Flow Index (MFI) - Real-time buying pressure and liquidity flow'}
              >
                <div className="flex items-center gap-1.5">
                  <span>{t.table.mfiShort || 'MFI %'}</span>
                  {sortField === 'mfi' && (sortDirection === 'asc' ? <ArrowUp className="w-3.5 h-3.5 text-emerald-500" /> : <ArrowDown className="w-3.5 h-3.5 text-emerald-500" />)}
                </div>
              </th>
              ...
              paginatedStocks.map((stock) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-3 min-w-[135px] font-mono">
                      {(() => {
                        const mfiAnalysis = calculateStockMfi(
                          stock.price,
                          stock.dayHigh,
                          stock.dayLow,
                          stock.open,
                        ...
                        const mfiVal = stock.mfi !== undefined ? stock.mfi : mfiAnalysis.mfi;
                        return (
                          <div 
                            className="flex flex-col gap-1 cursor-help"
                            title={lang === 'ar' ? mfiAnalysis.descriptionAr : mfiAnalysis.descriptionEn}
                          >
                            <div className="flex items-center justify-between gap-1.5">
                              <span className="font-bold text-xs sm:text-sm text-slate-800 dark:text-slate-100">
                                {mfiVal.toFixed(1)}%
                              </span>
                              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border leading-tight ${mfiAnalysis.badgeColorClass}`}>
                                {lang === 'ar' ? mfiAnalysis.statusLabelAr : mfiAnalysis.statusLabelEn}
                              </span>
                            </div>
                            {/* Micro Progress Bar */}
                            <div className="w-full bg-slate-200 dark:bg-slate-700/70 rounded-full h-1.5 overflow-hidden">
                              <div 
                                className={`h-full rounded-full bg-gradient-to-r ${mfiAnalysis.barColorClass} transition-all duration-500`}
                                style={{ width: `${Math.max(4, Math.min(100, mfiVal))}%` }}
                              />
                            </div>
                          </div>
                        );
                    ...
                    <td className="py-3 px-3">
                      <div className="relative w-24 flex items-center">
                        <input
                          ...
                          className="w-full px-2 py-1.5 rounded-lg text-xs font-mono bg-slate-50 dark:bg-[#0a0b0d] border border-slate-200 dark:border-slate-700 text-emerald-600 dark:text-emerald-400 placeholder-slate-400 focus:outline-none focus:border-emerald-500 transition-all font-bold pr-6 rtl:pr-2 rtl:pl-6"
                        />
                        {(upperEditVal !== '' || stock.upperAlert !== null) && (
                          <button
                            type="button"
                            onClick={() => {
                              handleAlertInputChange(stock.symbol, 'upper', '');
                              onUpdateAlerts(stock.symbol, null, stock.lowerAlert, stock.alertsEnabled);
                            }}
                            className="absolute right-1.5 rtl:right-auto rtl:left-1.5 text-slate-400 hover:text-rose-500 text-[11px] p-0.5 rounded-md hover:bg-slate-200 dark:hover:bg-slate-800 transition"
                            title={lang === 'ar' ? 'تصفير وحذف التنبيه العلوي' : 'Clear upper alert'}
                          >
                            ✕
                          </button>
                        )}
                    ...
                    <td className="py-3 px-3">
                      <div className="relative w-24 flex items-center">
                        <input
                          ...
                          onKeyDown={(e) => e.key === 'Enter' && handleSaveAlerts(stock)}
                          className="w-full px-2 py-1.5 rounded-lg text-xs font-mono bg-slate-50 dark:bg-[#0a0b0d] border border-slate-200 dark:border-slate-700 text-rose-600 dark:text-rose-400 placeholder-slate-400 focus:outline-none focus:border-rose-500 transition-all font-bold pr-6 rtl:pr-2 rtl:pl-6"
                        />
                        {(lowerEditVal !== '' || stock.lowerAlert !== null) && (
                          <button
                            type="button"
                            onClick={() => {
                              handleAlertInputChange(stock.symbol, 'lower', '');
                              onUpdateAlerts(stock.symbol, stock.upperAlert, null, stock.alertsEnabled);
                            }}
                            className="absolute right-1.5 rtl:right-auto rtl:left-1.5 text-slate-400 hover:text-rose-500 text-[11px] p-0.5 rounded-md hover:bg-slate-200 dark:hover:bg-slate-800 transition"
                            title={lang === 'ar' ? 'تصفير وحذف التنبيه السفلي' : 'Clear lower alert'}
                          >
                            ✕
                          </button>
                        )}
          ...
          <div className="bg-white dark:bg-[#161b22] border border-slate-200 dark:border-slate-700 rounded-2xl p-6 max-w-md w-full shadow-2xl text-slate-800 dark:text-slate-200">
            <div className="flex items-center gap-3 text-amber-500 dark:text-amber-400 mb-3">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-white font-sans">
                {t.actions.clearAllAlerts}
              </h3>
            </div>
            
            <p className="text-sm text-slate-600 dark:text-slate-300 font-sans mb-6">
              {t.actions.clearAllAlertsConfirm}
            </p>
            ...
            <div className="flex items-center justify-end gap-3 font-sans">
              ...
              <button
                id="btn-confirm-clear-all-alerts"
                onClick={() => {
                  setEditingAlerts({});
                  if (onClearAllAlerts) onClearAllAlerts();
                  setIsConfirmingClearAlerts(false);
                }}
                className="px-4 py-2 rounded-xl text-sm font-bold bg-amber-600 hover:bg-amber-500 text-white transition-all shadow-lg shadow-amber-600/20"
              >
                {t.actions.resetAlerts}
              </button>
            </div>
          </div>
        </div>
