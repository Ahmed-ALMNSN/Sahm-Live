...
export const StockDetailsModal: React.FC<StockDetailsModalProps> = ({
  ...
  return (
    ...
    >
      <div className="bg-[#0f1115] rounded-xl border border-slate-800 shadow-2xl w-full max-w-4xl max-h-[92vh] overflow-hidden flex flex-col font-sans animate-slide-in">
        
        {/* Header Bar */}
        <div className="px-5 py-3.5 border-b border-slate-800 flex items-center justify-between bg-[#161b22]">
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded ${isPositive ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'}`}>
              {isPositive ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" />}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-lg font-bold font-mono text-white tracking-wider">
                  {stock.symbol}
                </h1>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#0a0b0d] text-slate-400 font-mono border border-slate-700 uppercase">
        ...
        <div className="p-5 overflow-y-auto space-y-5">
          ...
          {/* Chart Section */}
          <div className="bg-[#161b22] rounded-lg p-4 border border-slate-800">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
              <div className="flex items-center gap-2">
                <BarChart2 className="w-4 h-4 text-emerald-400" />
                <span className="text-xs uppercase font-bold tracking-wider text-slate-200 font-mono">
              ...
              <div className="flex items-center gap-1 bg-[#0a0b0d] p-0.5 rounded border border-slate-700">
            ...
            <div className="h-64 sm:h-72 w-full relative">
              {isLoadingChart && (
                <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/60 backdrop-blur-xs rounded">
          ...
          {/* Key Statistics Grid */}
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300 mb-2.5 flex items-center gap-2 font-mono">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs">
              ...
              <div className="p-2.5 rounded bg-[#161b22] border border-slate-800">
                ...
                <span className="font-semibold text-slate-300 truncate block text-xs font-sans">
              ...
              <div className="p-2.5 rounded bg-[#161b22] border border-slate-800">
                ...
                <span className="font-mono text-emerald-400 font-bold block text-xs uppercase">
          ...
          <div className="p-4 rounded-lg bg-[#161b22] border border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <label className="inline-flex items-center gap-2 cursor-pointer text-xs font-semibold">
                ...
                />
                <span className="text-slate-300 font-mono">{t.table.active}</span>
              </label>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
              <div>
                <label className="block text-[10px] uppercase tracking-wider font-bold text-emerald-400 mb-1 font-mono">
                  {t.table.upperAlert} ↗
                </label>
                <input
                  type="number"
                  ...
                  className="w-full px-3 py-1.5 text-xs font-mono rounded bg-[#0a0b0d] border border-slate-700 text-white focus:outline-none focus:border-emerald-500"
                />
              </div>

              <div>
                <label className="block text-[10px] uppercase tracking-wider font-bold text-rose-400 mb-1 font-mono">
                  {t.table.lowerAlert} ↘
                </label>
                <input
                  type="number"
                  ...
                  className="w-full px-3 py-1.5 text-xs font-mono rounded bg-[#0a0b0d] border border-slate-700 text-white focus:outline-none focus:border-rose-500"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-1.5 rounded text-xs font-bold uppercase tracking-wider bg-emerald-600 hover:bg-emerald-500 active:scale-95 text-white shadow-sm transition-all flex items-center justify-center gap-1.5 font-mono"
              >
          ...
          {profile?.description && (
            <div className="text-xs text-slate-400 space-y-1">
              ...
              <p className="line-clamp-4 hover:line-clamp-none transition-all leading-relaxed font-sans text-slate-400">
                {profile.description}
              </p>
            </div>
          )}
